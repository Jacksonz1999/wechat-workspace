...
/* ---------------- 创建类 ---------------- */
def({
  name: 'task.create', desc: '新增工作任务', group: '创建',
  slots: [
  ...
  run(ctx, a) {
    const it = {
      ...
      due: a.date || D.today(), proj: '', done: false
    };
    ctx.data.work.tasks.unshift(it); touch(ctx, 'work');
    store.audit({ uid: ctx.uid, action: 'task.create', target: it.title, via: ctx.via });
    return { ok: true, text: '已添加任务：' + it.title + '\n截止 ' + D.dlName(it.due) + '，优先级 ' + it.pri, menu: ['今天要处理', '还有什么任务'] };
  }
});
