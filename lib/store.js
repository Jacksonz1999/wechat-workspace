...
// 数据目录优先级：显式 WB_DATA_DIR > Railway 挂载卷后自动注入的 RAILWAY_VOLUME_MOUNT_PATH > 默认 ../data
// 这样无论把卷挂到哪个路径，数据都会落在卷上，不用改代码。
...
function load() {
  ...
  // 持久化探针：首次启动时间戳。若 Volume 未挂载，每次部署该值都会变。
  if (!STATE._firstBootAt) { STATE._firstBootAt = new Date().toISOString(); save(); }
  return STATE;
}
/** 持久化诊断信息，供 /healthz 暴露 */
function bootInfo() {
  const s = load();
  const sz = (() => {
    try { return fs.statSync(STATE_FILE).size; } catch (e) { return -1; }
  })();
  let writable = null;
  try {
    ensureDir();
    ...
    writable = true;
  } catch (e) { writable = e.code || e.message; }
  return {
    firstBootAt: s._firstBootAt || null,
    sinceBootMs: s._firstBootAt ? Date.now() - new Date(s._firstBootAt).getTime() : null,
    stateFile: STATE_FILE,
    stateBytes: sz,
    users: Object.keys(s.users || {}).length,
    binds: Object.keys(s.binds || {}).length,
    dataDir: DATA_DIR,
    writable: writable,
    // Railway 挂载卷后会自动注入这两个变量；为空说明卷根本没挂上
    volume: {
      name: process.env.RAILWAY_VOLUME_NAME || null,
...
module.exports = {
  ...
  audit, readAudit, pullModules, pushModules, bootInfo
};
